...
const methodSignOut = async (): Promise<void> => {
  signOut(getAuth())
  router.push('/auth')
}
</script>
...
<template>
  <app-menubar :model="items" class="menu">
    ...
    <template #end>
      <span v-if="userStore.userId" class="flex align-items-center menu-exit">
        <span class="pi pi-sign-out p-p-menuitem-icon" />
        <span @click="methodSignOut" class="ml-2">Выход</span>
      </span>
    </template>
  </app-menubar>
</template>
